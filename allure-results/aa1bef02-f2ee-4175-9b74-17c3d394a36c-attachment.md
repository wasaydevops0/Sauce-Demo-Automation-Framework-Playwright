...
- Name: api\user.spec.js >> User API Tests using POM >> Delete user API Test
- Location: tests\api\user.spec.js:40:7
...
```ts
  ...
  34 |     token = tokenData.token; // Save token for subsequent tests
  35 |    
...
> 44 |     expect(deleteResponse.status()).toBe(200);
  ...
  66 |     const logoutToken = tokenData.token;
  67 | 
